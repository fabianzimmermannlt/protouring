...
// Read-only Abrechnung einer (gesperrten) Show – rendert einen self-contained Snapshot.
...
function CatBlock({ c }: { c: AbrechnungCategory }) {
  return (
    <>
      <tr style={{ fontWeight: 600 }}><td>{c.name}</td><td className="text-right" style={{ fontVariantNumeric: 'tabular-nums' }}>{M(c.total)}</td><td /></tr>
      {c.positions.map((p, i) => (
        <tr key={i}>
...
export default function AbrechnungView({ snap }: { snap: AbrechnungSnapshot }) {
  ...
  const neg = (() => { try { return new Decimal(snap.ergebnis || '0').isNegative() } catch { return false } })()
  return (
    <div className="pt-card" style={{ maxWidth: 760 }}>
      ...
      <div className="pt-card-body">
        ...
        <table className="data-table" style={{ width: '100%' }}>
          <thead>
            <tr><th>Position</th><th className="text-right" style={{ minWidth: 120 }}>Soll</th><th className="text-right" style={{ minWidth: 120, color: '#facc15' }}>Ist</th></tr>
          </thead>
          <tbody>
            <tr><td colSpan={3} style={{ fontWeight: 700, background: '#173a28', color: '#e5e7eb' }}>EINNAHMEN</td></tr>
            <tr><td>Gage (abzgl. Provision)</td><td className="text-right" style={{ fontVariantNumeric: 'tabular-nums' }}>{M(snap.gageNet)}</td><td /></tr>
            {income.map((c, i) => <CatBlock key={'i' + i} c={c} />)}
            <tr style={{ fontWeight: 600 }}><td>Summe Einnahmen</td><td className="text-right" style={{ fontVariantNumeric: 'tabular-nums' }}>{M(snap.sumEinnahmen)}</td><td /></tr>

            <tr><td colSpan={3} style={{ fontWeight: 700, background: '#26313f', color: '#e5e7eb' }}>AUSGABEN</td></tr>
            {expense.map((c, i) => <CatBlock key={'e' + i} c={c} />)}
            <tr style={{ fontWeight: 600 }}><td>Summe Ausgaben</td><td className="text-right" style={{ fontVariantNumeric: 'tabular-nums' }}>{M(snap.sumAusgaben)}</td><td /></tr>

            <tr style={{ fontWeight: 700, background: '#2f2f2f' }}>
              <td>ERGEBNIS</td>
              <td className="text-right" style={{ fontVariantNumeric: 'tabular-nums', color: neg ? '#f87171' : '#4ade80' }}>{M(snap.ergebnis)}</td>
              <td />
            </tr>
            <tr style={{ color: '#9ca3af' }}><td>je Bandmitglied ({snap.memberCount})</td><td className="text-right" style={{ fontVariantNumeric: 'tabular-nums' }}>{M(snap.jeBandmitglied)}</td><td /></tr>
          </tbody>
        </table>
      </div>
    </div>
